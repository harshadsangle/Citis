...
let CertificateService = class CertificateService {
    ...
    certificateNumber() {
        return `CITIS-${new Date().getUTCFullYear()}-${(0, node_crypto_1.randomBytes)(5).toString("hex").toUpperCase()}`;
    }
    verificationId() {
        return (0, node_crypto_1.randomBytes)(18).toString("base64url");
    }
    async certificateRow(id, user) {
        const result = await this.db.query(`${certificateSelect}
       WHERE cert.id = $1 AND cert.tenant_id = $2 AND cert.status = 'ISSUED'`, [id, user.tenantId]);
        const row = result.rows[0];
        if (!row)
            throw new common_1.NotFoundException("Certificate not found.");
        (0, access_scope_1.assertScopeForRead)(user, String(row.institution_id), row.campus_id);
        return row;
    }
    async assertCertificateAccess(row, user) {
        ...
        if (user.roles.some((role) => role.code === "TEACHER")
            ...
            const assigned = await this.db.query(`SELECT 1 FROM lms_instructor_assignments
         ...
         LIMIT 1`, [user.tenantId, row.institution_id, row.course_id, row.campus_id ?? null, user.id]);
            if (!assigned.rows[0])
                throw new common_1.NotFoundException("Certificate not found.");
        }
    }
    ...
    async list(user, page, pageSize, offset, query = {}) {
        ...
        if (!staffRole(user) || (!administrator && !(0, access_scope_1.isPlatformUser)(user))) {
            values.push(user.id);
        ...
        if (!(0, access_scope_1.isPlatformUser)(user) && user.roles.some((role) => role.code === "TEACHER") && !administrator) {
            values.push(user.id);
            clauses.push(`EXISTS (
       ...
       ORDER BY cert.issue_date DESC, cert.id ASC`, values);
    ...
    async verify(identifier, requestId) {
    ...
    async issueIfEligible(tenantId, courseId, learnerId, request) {
        const issued = await this.db.transaction(async (client) => {
            const enrollment = await client.query(`SELECT e.id AS enrollment_id, e.tenant_id, e.institution_id, e.campus_id, e.course_id,
         ...
         LIMIT 1`, [tenantId, courseId, learnerId]);
            const eligibleEnrollment = enrollment.rows[0];
            if (!eligibleEnrollment)
                return null;
            const inserted = await client.query(`INSERT INTO lms_certificates
         ...
         RETURNING *`, [
            ...
            if (inserted.rows[0])
                return { ...eligibleEnrollment, ...inserted.rows[0], issued_now: true };
            const existing = await client.query(`${certificateSelect}
         WHERE cert.tenant_id = $1 AND cert.enrollment_id = $2 AND cert.status = 'ISSUED'`, [tenantId, eligibleEnrollment.enrollment_id]);
            return existing.rows[0] ? { ...existing.rows[0], issued_now: false } : null;
        });
        if (issued && request && issued.issued_now) {
            await this.audit.record({
                tenantId,
                institutionId: issued.institution_id,
                campusId: issued.campus_id ?? null,
