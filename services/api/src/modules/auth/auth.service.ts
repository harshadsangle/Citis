...
export class AuthService {
  ...
  async login(input: LoginDto, metadata: { ipAddress?: string; userAgent?: string; requestId?: string }) {
    ...
    const diagnosticError = (error: unknown) => {
      ...
      console.error(
        `[TEMP_AUTH_DIAGNOSTIC] AuthService.login failed requestId=${requestId} stage=${stage} errorName=${errorName} errorCode=${errorCode} errorMessage=${errorMessage}`,
      );
    };
    ...
    try {
      ...
      diagnostic(
        `stage=user-lookup status=${user ? "found" : "not-found"} rowCount=${result.rows.length} userStatus=${user?.status || "none"} passwordHashPresent=${Boolean(user?.password_hash)}`,
      );
