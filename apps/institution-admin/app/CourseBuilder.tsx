...
function newCourseCodeSeed() {
  return globalThis.crypto?.randomUUID?.() || newId("course");
}

function courseCodeFromSeed(seed: string) {
  return `CRS-${seed.replaceAll("-", "").toUpperCase()}`;
}
...
export default function CourseBuilder({
  ...
  const [courseCodeSeed] = useState(newCourseCodeSeed);
  ...
  function reviewErrors(prefix: string) {
    return Object.entries(validationErrors)
      .filter(([key]) => key === prefix || key.startsWith(`${prefix}.`))
      .map(([key, message]) => <span className="builder-review-error" key={key}>{message}</span>);
  }
  ...
  function showValidationErrors(errors: ValidationErrors, keepCurrentStep = false) {
  ...
  async function createCourse() {
    ...
    try {
      ...
      const response = await requestMultipartWithProgress<ApiResponse>(apiBase, "/course-builder", formData, (loaded, total) => {
      ...
      if (Object.keys(apiErrors).length > 0) showValidationErrors(apiErrors, step === 3);
