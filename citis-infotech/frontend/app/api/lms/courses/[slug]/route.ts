...
async function canManageCourse(slug: string) {
  ...
  if (!user) return { response: NextResponse.json({ error: "Sign in to manage courses." }, { status: 401 }), user: null };
  if (user.role !== "instructor" && user.role !== "admin") return { response: NextResponse.json({ error: "Only instructors and admins can manage courses." }, { status: 403 }), user: null };
  ...
  if (!course) return { response: NextResponse.json({ error: "Course not found." }, { status: 404 }), user: null };
  if (user.role === "instructor" && course.instructor_id !== null && String(course.instructor_id) !== String(user.id)) {
    return { response: NextResponse.json({ error: "You can only manage your own courses." }, { status: 403 }), user: null };
  }
