...
export default function LmsDashboardPage() {
  return (
    <section className="bg-[linear-gradient(180deg,#eef7fa_0%,#f7fbfc_28rem,#f7fbfc_100%)] py-10 sm:py-14">
      <div className="container-site">
        <LmsDashboardGate>
        <div className="flex flex-col justify-between gap-6 rounded-3xl border border-[#cfe3ea] bg-white/80 p-6 shadow-[0_12px_35px_rgba(18,61,92,0.05)] sm:flex-row sm:items-end sm:p-8">
          <div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => {
            ...
            return <Card key={stat.label} className="border-[#cfe3ea] shadow-[0_8px_24px_rgba(18,61,92,0.04)]"><CardContent className="flex items-center justify-between p-5"><div><p className="text-sm text-muted-foreground">{stat.label}</p><p className="mt-2 font-heading text-3xl font-bold tracking-tight text-[#123d5c]">{stat.value}</p><p className="mt-1 text-xs font-semibold text-primary">{stat.note}</p></div><span className="grid size-11 place-items-center rounded-xl bg-primary/10 text-primary"><Icon className="size-5" /></span></CardContent></Card>;
          })}
        </div>

        <div className="mt-8"><MyEnrolledCourses /></div>
