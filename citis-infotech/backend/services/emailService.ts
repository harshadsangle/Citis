...
export const sendCareerApplication = (application: {
  name: string; email: string; phone?: string; resume: string; coverLetter?: string;
}, careerTitle: string) => send(
  process.env.ADMIN_EMAIL || process.env.SMTP_USER || '',
...
export const sendPasswordReset = (email: string, token: string) => {
  const url = `${process.env.CLIENT_URL || 'http://localhost:3000'}/reset-password/${encodeURIComponent(token)}`;
  return send(email, 'Reset your password', `<p>This link expires in one hour.</p><p><a href="${url}">Reset password</a></p>`);
};

export const sendVerification = (email: string, token: string) => {
  const url = `${process.env.CLIENT_URL || 'http://localhost:3000'}/verify-email/${encodeURIComponent(token)}`;
  return send(email, 'Verify your email', `<p><a href="${url}">Verify email address</a></p>`);
};

export const sendWelcome = (email: string, name: string) =>
  send(email, 'Welcome to CITIS InfoTech', `<p>Welcome, ${escapeHtml(name)}. Your email has been verified.</p>`);
