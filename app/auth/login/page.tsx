    ...
    <section className="relative isolate min-h-[calc(100vh-var(--header-height))] overflow-hidden py-16">
      ...
      <div className="container-site grid items-stretch gap-8 lg:grid-cols-2">
        <div className="brand-gradient hidden min-h-[34rem] flex-col justify-between overflow-hidden rounded-3xl p-10 text-white shadow-2xl lg:flex">
          ...
          <div><p className="text-xs font-bold tracking-[0.2em] text-orange-300 uppercase">Learning portal</p><h1 className="mt-5 font-heading text-4xl leading-tight font-semibold">Continue building capability that moves with you.</h1><p className="mt-5 leading-7 text-blue-100">Access your learning paths, projects, mentor feedback, credentials, and community.</p></div>
        ...
        <div className="surface flex items-center rounded-3xl p-6 sm:p-10"><div className="mx-auto w-full max-w-md"><Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-primary"><ArrowLeft className="size-4" />Back to CITIS</Link><h1 className="mt-9 font-heading text-3xl font-semibold">Welcome back</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Sign in to continue your CITIS learning experience.</p><div className="mt-8"><LoginForm /></div><p className="mt-7 text-center text-sm text-muted-foreground">Need access? <Link href="/contact" className="font-semibold text-primary hover:underline">Contact your programme team</Link></p></div></div>
      </div>
    </section>
  );
}
