      ...
      <article>
        <header className="relative overflow-hidden border-b border-border bg-[#eef3f8] py-16 text-slate-900 sm:py-24">
          ...
          <div className="container-site relative">
            ...
            <div className="mt-8 max-w-4xl"><Badge variant="secondary">{post.category}</Badge><h1 className="mt-5 font-heading text-4xl leading-tight font-semibold tracking-tight text-balance text-slate-900 dark:text-slate-900 sm:text-5xl lg:text-6xl">{post.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-600 dark:text-slate-600">{post.excerpt}</p><div className="mt-7 flex flex-wrap items-center gap-5 text-sm text-slate-600 dark:text-slate-500"><span>{post.author}</span><span className="flex items-center gap-1.5"><CalendarDays className="size-4" />{new Date(post.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</span><span className="flex items-center gap-1.5"><Clock className="size-4" />{post.readTime}</span></div></div>
          </div>
        </header>
        ...
        <div className="container-site grid gap-10 py-14 lg:grid-cols-[minmax(0,1fr)_13rem] lg:gap-16 lg:py-20">
          ...
          <aside className="lg:sticky lg:top-28 lg:h-fit"><p className="text-xs font-bold tracking-wider text-muted-foreground uppercase">Share this article</p><div className="mt-4 flex gap-2 lg:flex-col"><a className="inline-flex h-10 items-center gap-2 rounded-lg border border-border px-3 text-sm text-slate-700 hover:text-primary dark:text-foreground" href={`https://www.linkedin.com/sharing/share-offsite/?url=${url}`} target="_blank" rel="noreferrer"><FaLinkedinIn className="size-4" /><span className="hidden lg:inline">LinkedIn</span></a><a className="inline-flex h-10 items-center gap-2 rounded-lg border border-border px-3 text-sm text-slate-700 hover:text-primary dark:text-foreground" href={`https://twitter.com/intent/tweet?url=${url}&text=${title}`} target="_blank" rel="noreferrer"><FaXTwitter className="size-4" /><span className="hidden lg:inline">X / Twitter</span></a><a className="inline-flex h-10 items-center gap-2 rounded-lg border border-border px-3 text-sm text-slate-700 hover:text-primary dark:text-foreground" href={`mailto:?subject=${title}&body=${url}`}><LinkIcon className="size-4" /><span className="hidden lg:inline">Email link</span></a></div></aside>
        </div>
        </div>
      </article>
      <section className="border-y border-border bg-[#e8edf5] py-16 dark:bg-slate-900"><div className="container-site"><h2 className="font-heading text-3xl font-semibold">Continue exploring</h2><div className="mt-8 grid gap-5 md:grid-cols-3">{related.map((item) => <Card key={item.slug} className="group h-full text-slate-900 dark:text-card-foreground"><CardContent className="p-6"><Badge variant="outline">{item.category}</Badge><h3 className="mt-4 font-heading text-lg font-semibold text-slate-900 dark:text-card-foreground">{item.title}</h3><p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-600 dark:text-muted-foreground">{item.excerpt}</p><Link href={`/highlights/blogs/${item.slug}`} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary dark:text-primary">Read article<ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link></CardContent></Card>)}</div></div></section>
